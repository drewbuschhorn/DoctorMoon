#!/bin/bash
sudo apt-get install python-dev libfreetype6-dev libpng-dev libxslt-dev libxml2-dev libpq-dev postgresql python-pip python-dev build-essential graphviz graphviz-dev libgraphviz-dev screen
...
echo "Setting the postgres user password.  Recommend django13 for dev password, but this is highly insecure.  Will also reset any other application using postgres on this machine."
echo "Proceed? [Y/N]"
read a
if [[ $a == "Y" || $a == "y" || $a = "" ]]; then
